...
* Babel
Babel extends Org-mode [fn:: http://orgmode.org] a Major mode of the
Emacs text editor [fn:: http://www.gnu.org/software/emacs/] with the
ability to execute and selectively export embedded source code blocks.
The results of code execution -- text, tables and graphics -- can be
used as input to other source code blocks or passed to the powerful
publishing facilities of Org-mode.

Babel provides a single environment supporting
...
- the storage and analysis of raw data in both spreadsheet and linked
  from external files
- sophisticated organizational and task management facilities

The core of Babel is language agnostic and can easily be extended to
provide support for new programming languages.  Babel currently
supports over 20 programming languages, with most language support
contributed by Babel users.

The majority of the features above are provided directly by the
Org-mode project which Babel extends.  As such the following
description of Babel will begin with a description of Org-mode.
