...
\ref{background}).  We describe the design and usage of Babel
...
*** Evaluation
Babel knows how to evaluate code written in a number of languages.  In the
evaluation of source code Babel makes use of a great deal of existing
Emacs support for interaction with languages running as both external and inferior (Emacs-internal)
processes.
