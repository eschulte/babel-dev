...
* Design
  ...
  :CUSTOM_ID: design
  :END:
The babel environment is developed in the Lisp language, an extensible
language that is itself a Lisp program.  Lisp promotes a style of
programming that has been characterized as "bottom-up" because the
language itself changes in response to the particular problem being
solved.  At the core of Babel is the Emacs text editor and emacs-lisp,
...
for keeping notes, maintaining lists of items to do, and planning
...
named and referred to elsewhere in the document and that code blocks
are executable.  The results of code block evaluation can be written
...
its parent language, Babel is extensible: support for new languages
can be added by writing emacs-lisp functions, extending Babel in the
traditional Lisp way.  Additionaly, useful code blocks can be added to
a Library of Babel, where they can be called from any Org-mode
document.
...
languages like LaTeX or HTML.   Through living inside of (and standing on
...
# Discuss constructs that are recognized as data by Babel--examples (non-code),
# tables and links
...
** Code and Data Blocks
    ...
    :END:
Babel extends Org-mode with the ability to activate certain constructs
with a simple naming protocol that identifies the construct as either
a data block or a code block.  In addition, the syntax of Org-mode
code blocks is extended to facilitate evaluation.  Babel writes the
results of source code evaluation to the document as Org-mode
constructs.
...
#+begin_comment 
# Most of this material appears above
    
With Babel, Org-mode documents become environments for computation as
well as containers for text, code and data.  Certain components of
...
Babel recognizes Org-mode tables, examples, and links as data block
content.  Data blocks in Babel are preceded by a line that begins with
 =#+results:=, followed by a name unique within the
document, e.g.,
     #+begin_src org
       ,#+results: linked-data
       ,[[http://external-data.org]]
     #+end_src
Alternatively, tables can be preceded with a =#+tblname:= line, e.g.,
     #+begin_src org
       ,#+tblname: numbered-primes
       ,| 1 |  2 |
       ,| 2 |  3 |
       ,| 3 |  5 |
       ,| 4 |  7 |
...
- example data :: Org-mode /example/ blocks are named with a =#+results:=
     line.   e.g.,  For example, the
...
# Attempting to focus discussion here on what Babel adds to Org-mode
# code blocks, assuming Org-mode code blocks will be described in the
# preceding section
...
# reproducible research.
#
# I agree, this is a good lead into the following point-by-point
# discussion of the strengths and weaknesses of babel (Eric)
...
# More concluding material culled from an earlier section
