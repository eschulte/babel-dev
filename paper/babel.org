...
#+end_comment
By default, the results of evaluating code blocks are inserted in the
Org-mode buffer as protected text immediately after the code block.
In practice the user is given significant control over the handling of
code block results both during interactive evaluation and during
export.  At the most basic level, results can be collected from code
blocks by value or as output.  This behavior is controlled by the
...
The difference can be demonstrated with the following bit of =perl=
...
** Live Climate Data
Alright, a revised approach, using this [[ftp://ftp.ncdc.noaa.gov/pub/data/ghcn/v2/v2.temperature.readme][noaa data]].
