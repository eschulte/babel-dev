...
advantage of this facility.   Adding a
=:session= /header argument/ causes Babel to 
#+begin_comment
#+begin_src org
...
- =:results value= :: Specifies that the code block should be treated
     ...
     function.  This is the default setting.  Results returned as
     values can be of any type supported by Babel.
#+begin_comment
  The following block
...
- =:results output= :: Specifies that the results should be collected
     ...
     application responsible for code execution, and placed in the
     Org-mode buffer as a string or an Org-mode link.
...
Results returned as values can take a number of different forms.  So far we have only
...
The following diagram, which illustrates the relationship of Babel to
some other literate programming and reproducible research software
tools, was created in a few minutes.  The header argument =:file= is
used to specify the name of the =ditaa= output and =:cmdline= passes
parameters to =ditaa=.

# The line leading up from the WEB box doesn't have a clean junction
# at the top for reasons that escape me.
#+BEGIN_SRC ditaa :file lp-rr.png :cmdline -r -s 0.8
            +--------+     +--------+
            |        |     |        |
Multiple    | noweb  |     |  Babel |
            | cBLU   |     |  cGRE  |
            +----+---+     +--+--+--+
                 |            |  |  
                 +-----+------+  +----+
...
Single             |  WEB   |     | Sweave |
                   | cBLU   |     | cYEL   |
                   +--------+     +--------+
Language           Literate       Reproducible
