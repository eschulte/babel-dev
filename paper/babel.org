...
documents, effectively dividing them into text, code, and data
sections.  Activation means that code sections are executable and that
both they and data sections can be named and referred to elsewhere in
the document.  In this way, an Org-mode buffer becomes a place where
...
can be added by writing emacs-lisp functions, and useful code blocks
in any language can be added to a Library of Babel, where they can be
called from any Org-mode document.
