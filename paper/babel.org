...
#+begin_src org
  ...
  ,#+begin_src haskell
  ,  zip [1, 2, 3, 4, 5] (map (\ x -> x + 1) [1, 2, 3, 4, 5])
  ,#+end_src
...
into Org-mode buffers in the form of /scalars/, /tables/ and links to
external files.  It is also possible for data to flow from each of
these containers (/scalars/, /tables/ and /links/) into code blocks,
and even for data to pass between different code blocks without ever
...
As the previous two examples demonstrate, strings will be passed in a
strings, numbers as numbers, and as shown in Figure \ref{some-table}
tables will be passed in as tables.
#+source: some-table
#+begin_src org
  ,#+results: table-x
  ,| 1 | 
...
scripts written in a shell or python with statistical analysis written
in R.  The goals of reproducible research are only met when such an
integration is both easily accomplished by the initial researcher, and
preserved in a distributable container (like a plain text document).

#+label: too-much
#+begin_src org
  ,would be great to have an example here that (in 4 different languages)
  ,1) grabbed a list of items from a table
  ,2) somehow grabbed data related to those items from the web
  ,3) pre-processed the results
  ,4) did some analysis in R and then spat out the results
#+end_src
...
Borrowing term from the Literate Programming community Babel supports
...
- tangling :: Tangling consists not only of extracting source code
     ...
     the reference process \ref{noweb}.  Babel implements the same
