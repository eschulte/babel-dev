...
* Introduction
  ...
  2. Code blocks as functions: they can be parameterised, can refer to
     other code blocks, and can be called remotely.
...
* Getting started
  ...
  2) Make sure that the path to org's contrib/lisp directory is in
     your load-path and add the following to your =.emacs= and
     evaluate.
...
*** Source code blocks
    ...
    containing [[http://www.ruby-lang.org/][ruby]] code (if you don't have ruby installed the ruby
    examples won't work, so you can try the shell script alternative):
...
For longer snippets of code, it is advisable to use the function
=org-edit-src-code= (=C-c '=).  This gives you a new buffer that
contains the source code with the appropriate mode turned on.
...
#+resname: name
: Hello, today's date is Thu Oct  8 09:45:53 2009
: Two plus two is
...
*** Session-based evaluation
    ...
    data objects that persist between different code blocks. Org-babel
    supports evaluation of code within such sessions with the
    =:session= header argument. If the header argument is given a
    value then that will be used as the name of the session.
...
*** Summary of results output/value & sessions

    The following notes apply particularly to perl, python, R and
    ruby.
...
**** :results value (non-session)
     This is the default. The value is obtained by wrapping the code
     in a function and evaluating that function. Therefore, code
     should be written as if it were the body of such a function. In
     particular, note that languages such as python and perl do not
...
**** :results output (non-session)
     ...
     the value returned is the (text) contents of stdout.

**** :results value (session)
     The code is passed to the interpreter running as an emacs
     ...
     language-specific manner (the value of the variable =_= in python
...
**** :results output (session)
     ...
     sequence of (text) output from the interpreter.

Note that in ':results value' (session and non-session), and attempt
is made to interpret the result as a table (a one- or two-dimensional
vector of strings or numbers) where appropriate.
...
*** Inline source blocks
    Code can be evaluated inline using the following syntax:
...
of evaluating the source-code block named 'example-block'.

: # <<example-block()>>

Any optional arguments can be passed to example-block by placing the
arguments inside the parenthesis following the convention defined when
...
**** Language specific documentation
     Examples, special header arguments and guides on how to use it
     are available here for the following languages:
...
- =:results= :: options supplied after the =:results= header argument
             specify what should be done with the output of
             source-code blocks
  - The following options are mutually exclusive, and specify how the
    results should be collected from the source-code block
    - value :: The result is the /value/ of the last statement in the
               source block
    - output :: The result is a concatenation of the text that is
                output by the source block (currently, this includes
                error output).
  - The following options are mutually exclusive and specify what type
    of results the code block will return
    - table, vector :: the results should be interpreted as a one- or
                       two-dimensional vector (so a "single number" is
                       a one-dimensional vector of length one), and
                       will be inserted into the org-mode file as a
                       table
    - scalar :: the results should be interpreted as a scalar value,
                and will be inserted into the org-mode file as quoted
                text
    - file :: the results should be interpreted as the path to a file,
              and will be inserted into the org-mode file as a link
    - raw, org :: the results are interpreted as raw org code and are
             inserted directly into the buffer
    - html :: results enclosed in a begin_html block
    - latex :: results enclosed in a begin_latex block
    - code :: the result value is represented as parseable code
    - pp :: the result value is represented as pretty-printed code
            (emacs-lisp, python, ruby)
