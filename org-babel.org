...
**** :results
     There are three types of results header arguments only one option
     from each type may be supplied per source block.
     1) those specifying how the results should be *collected* from
        the source code block
     2) those specifying what *type* of result the source code block
        will return -- which has implications for /how/ they will be
        inserted into the org-mode buffer
     3) and those specifying how these results should be *handled*
...
***** collection
     ...
     - value :: The result is the /value/ of the last statement in the
          source block.  In some languages (e.g. python) usage of this
          result type will require that a =return= statement be
          included in the body of the source code block.
     - output :: The result is the collection of everything printed
          (i.e. to STDOUT) during the execution of the source code
          block.
...
***** type
     ...
     type of results the code block will return
      ...
      - table, vector :: The results should be interpreted as table --
           so if a "single number" is returned it will be converted
           into a table with one row and one column.
      - scalar, verbatim :: The results should be interpreted
           literally -- meaning it will not be converted into a table.
           The result will be inserted into the org-mode file as
           quoted text.
      - file :: The results will be interpreted as the path to a file,
           and will be inserted into the org-mode buffer as a file
           link.
      - raw, org :: The results are interpreted as raw org code and
      ...
      - pp :: The result is converted to pretty-printed code
           (currently supported by emacs-lisp, python, and ruby) and
           is enclosed in a code block.
